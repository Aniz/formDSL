...
def get_form_mm(debug=False):
    ...
    entity_mm = metamodel_from_file(join(this_folder, 'form.tx'),
                                    classes=[SimpleType],
...
def main(debug=False):
    ...
    entity_mm = get_form_mm(debug)

    # Export to .dot file for visualization
    dot_folder = join(this_folder, 'dotexport')
    if not os.path.exists(dot_folder):
        os.mkdir(dot_folder)
    metamodel_export(entity_mm, join(dot_folder, 'form_meta.dot'))

    # Build Person model from form.ev file
    form_model = entity_mm.model_from_file(join(this_folder, 'form.form'))

    # Export to .dot file for visualization
    model_export(form_model, join(dot_folder, 'form.dot'))
